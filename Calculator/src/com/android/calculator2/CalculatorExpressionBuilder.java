...
//Innovatech_property
    //xuzhenjun add for calclator add zero before dot
    private static boolean mCalcDotAddZeroBefore = SystemProperties.getBoolean("ro.calc.dot.add.zero.before",false);
            ...
            switch (appendExpr.charAt(0)) {
                case '.':
                    ...
                    if (index != -1 && index < start && TextUtils.isDigitsOnly(expr.substring(index + 1, start))) {
                        appendExpr = "";
                    //Innovatech-xuzhenjun add for calclator add zero before dot start
                    }else if(mCalcDotAddZeroBefore && (start == 0 ||!TextUtils.isDigitsOnly(expr.substring(start - 1, start)))){
			appendExpr = "0.";
                    }
                    //Innovatech-xuzhenjun add for calclator add zero before dot end
                    break;
                case '+':
                case '*':
                case '/':
